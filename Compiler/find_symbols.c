...
int parse_function_header(char *header, int lineNum, char *funName)
{
...
int parse_line(char *line, int lineNum)
{
  ...
  if(strcmp(strings[0], "int") == 0){
    for(j = 1; j < 10; j++){
      char c = strings[j][0];
      if((c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')){
	if(strcmp(strings[j - 1], "=") && strcmp(strings[j - 1], "+") && strcmp(strings[j - 1], "-")){
